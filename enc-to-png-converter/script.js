...
document.addEventListener('DOMContentLoaded', function () {
    ...
    let selectedFiles = [];
    let convertedImages = [];
    let currentMode = 'enc-to-png'; // 'enc-to-png' or 'png-to-enc'
    ...
    // Mode buttons
    const encToPngBtn = document.getElementById('encToPngBtn');
    const pngToEncBtn = document.getElementById('pngToEncBtn');
    ...
    // Mode switcher
    encToPngBtn.addEventListener('click', () => switchMode('enc-to-png'));
    pngToEncBtn.addEventListener('click', () => switchMode('png-to-enc'));
    ...
    function switchMode(mode) {
        ...
        // Update UI
        encToPngBtn.classList.toggle('active', mode === 'enc-to-png');
        pngToEncBtn.classList.toggle('active', mode === 'png-to-enc');
        ...
        if (mode === 'enc-to-png') {
            ...
            convertBtnText.textContent = 'Convert to PNG';
            passwordLabel.textContent = 'Decryption Key/Password (if required):';
        } else {
            fileInput.setAttribute('accept', '.png,image/png');
            uploadTitle.textContent = 'Drag & Drop PNG files here';
            uploadInfo.textContent = 'Supports PNG image formats';
    ...
    function displayFileList() {
        ...
        selectedFiles.forEach((file, index) => {
            ...
            const fileExt = currentMode === 'enc-to-png' ? 'ENC' : 'PNG';
            fileItem.innerHTML = `
            <div class="file-info">
    ...
    async function convertFiles() {
        ...
        if (currentMode === 'png-to-enc' && !password) {
        ...
        for (let i = 0; i < totalFiles; i++) {
            ...
            try {
                ...
                if (currentMode === 'enc-to-png') {
                    convertedData = await convertEncToPng(file, password);
                    outputName = file.name.replace('.enc', '.png');
                } else {
                    convertedData = await convertPngToEnc(file, password);
                    outputName = file.name.replace(/\.(png)$/i, '.enc');
                }
                ...
                convertedImages.push({
                    ...
                    isImage: currentMode === 'enc-to-png'
                });
            } catch (error) {
    ...
    // ENC to PNG Conversion
    async function convertEncToPng(file, password = '') {
    ...
    async function convertPngToEnc(file, password) {
        return new Promise((resolve, reject) => {
            const reader = new FileReader();
    ...
    function displayResults() {
        ...
        const mode = currentMode === 'enc-to-png' ? 'PNG' : 'ENC';
