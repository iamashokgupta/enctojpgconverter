...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function switchMode(mode) {
        ...
        if (mode === 'enc-to-jpg') {
            ...
            convertBtnText.textContent = 'Convert to JPEG';
            passwordLabel.textContent = 'Decryption Key/Password (if required):';
        } else {
            fileInput.setAttribute('accept', '.jpg,.jpeg,.jpe,.jfif,image/jpeg');
            uploadTitle.textContent = 'Drag & Drop JPEG/JPEG files here';
            uploadInfo.textContent = 'Supports JPEG, JPEG, JPE, JFIF image formats';
    ...
    function handleFiles(files) {
        ...
        fileArray.forEach(file => {
            if (currentMode === 'enc-to-jpg') {
                ...
                // Check for JPEG/JPEG files
                if (file.type.startsWith('image/jpeg') || file.type.startsWith('image/jpg') ||
                    file.name.toLowerCase().match(/\.(jpg|jpeg|jpe|jfif)$/)) {
                    selectedFiles.push(file);
                } else {
                    showNotification('Please select JPEG/JPEG files only', 'error');
                }
            }
        });
    ...
    function displayFileList() {
        ...
        selectedFiles.forEach((file, index) => {
            ...
            const fileExt = currentMode === 'enc-to-jpg' ? 'ENC' : 'JPEG';
            fileItem.innerHTML = `
            <div class="file-info">
    ...
    async function convertFiles() {
        ...
        // Validate password for JPEG to ENC mode
        ...
        for (let i = 0; i < totalFiles; i++) {
            ...
            try {
                ...
                if (currentMode === 'enc-to-jpg') {
                    ...
                    outputName = file.name.replace('.enc', '.jpeg');
                } else {
    ...
    function displayResults() {
        ...
        const mode = currentMode === 'enc-to-jpg' ? 'JPEG' : 'ENC';
...
`;
    ...
    const preloadedReviews = [
        {
            ...
            text: 'I had over 200 WhatsApp backup .enc files that I couldn\'t open. This tool converted every single one to perfect JPEGs in under a minute. Absolutely amazing!',
    ...
    console.log('ENC ⇄ JPEG Converter initialized');
