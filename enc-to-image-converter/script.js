...
document.addEventListener('DOMContentLoaded', function () {
    ...
    let selectedFiles = [];
    let convertedImages = [];
    let currentMode = 'enc-to-image'; // 'enc-to-image' or 'image-to-enc'
    ...
    // Mode buttons
    const encToImageBtn = document.getElementById('encToImageBtn');
    const imageToEncBtn = document.getElementById('imageToEncBtn');
    ...
    // Mode switcher
    encToImageBtn.addEventListener('click', () => switchMode('enc-to-image'));
    imageToEncBtn.addEventListener('click', () => switchMode('image-to-enc'));
    ...
    function switchMode(mode) {
        ...
        // Update UI
        encToImageBtn.classList.toggle('active', mode === 'enc-to-image');
        imageToEncBtn.classList.toggle('active', mode === 'image-to-enc');

        // Update file input accept attribute
        if (mode === 'enc-to-image') {
            ...
            convertBtnText.textContent = 'Convert to Image';
            passwordLabel.textContent = 'Decryption Key/Password (if required):';
        } else {
            fileInput.setAttribute('accept', 'image/*');
            uploadTitle.textContent = 'Drag & Drop Image files here';
            uploadInfo.textContent = 'Supports JPG, PNG, WEBP, GIF, and other image formats';
    ...
    function displayFileList() {
        ...
        selectedFiles.forEach((file, index) => {
            ...
            const fileExt = currentMode === 'enc-to-image' ? 'ENC' : 'Image';
            fileItem.innerHTML = `
            <div class="file-info">
    ...
    async function convertFiles() {
        ...
        if (currentMode === 'image-to-enc' && !password) {
        ...
        for (let i = 0; i < totalFiles; i++) {
            ...
            try {
                ...
                if (currentMode === 'enc-to-image') {
                    convertedData = await convertEncToImage(file, password);
                    outputName = file.name.replace('.enc', '.png');
                } else {
                    convertedData = await convertImageToEnc(file, password);
                    outputName = file.name.replace(/\.(jpg|jpeg|png|gif|webp|bmp)$/i, '.enc');
                }
                ...
                convertedImages.push({
                    ...
                    isImage: currentMode === 'enc-to-image'
                });
            } catch (error) {
    ...
    // ENC to Image Conversion
    async function convertEncToImage(file, password = '') {
    ...
    async function convertImageToEnc(file, password) {
        return new Promise((resolve, reject) => {
            const reader = new FileReader();
    ...
    function displayResults() {
        ...
        const mode = currentMode === 'enc-to-image' ? 'Image' : 'ENC';
